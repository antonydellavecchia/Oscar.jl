...
Returns a sorted vector of all numbers to $2 \cdot 10^6$ that occur as sizes
of perfect groups.

# Examples
```jldoctest
julia> sizes_perfect_groups()[1:10]
...
function perfect_group(::Type{T}, n::IntegerUnion, k::IntegerUnion) where T <: GAPGroup
   ...
   k >= 1 || throw(ArgumentError("group index must be positive, not $m"))
...
# Examples
```jldoctest
julia> perfect_identification(alternating_group(5))
(60, 1)

julia> perfect_identification(SL(2,7))
(336, 1)
```
"""
...
julia> argmax(number_perfect_groups, sizes_perfect_groups())
1966080

julia> number_perfect_groups(ans)
7344
```
"""
